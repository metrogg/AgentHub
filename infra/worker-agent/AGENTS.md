...
Your home directory (`~/`) is your agent workspace — SOUL.md, openclaw.json, memory/, skills/ all live here. Shared files live at the shared storage path configured in your environment.

- **Your agent files:** `~/` (SOUL.md, openclaw.json, memory/, skills/)
- **Shared space:** configured via `AGENTHUB_SHARED_STORAGE_ROOT` (tasks, knowledge, collaboration data)
...
- **@mention must use full Matrix ID** (with domain) — e.g. `@manager:agenthub.local`. Never write `${MATRIX_DOMAIN}` literally in a message
...
- **Builtin skills** (e.g. `task-progress/`, `file-sync/`) — assigned by your coordinator. **Do not modify these.**
- **Custom skills** — skills you create or that came from your package. You can freely add and modify these.

Each skill directory contains a `SKILL.md` explaining how to use it. Read the relevant `SKILL.md` before using a skill.

## Communication

You live in a Matrix Room with a **human admin** and your **coordinator**:
- **Your Worker Room** (`Worker: <your-name>`): private room (admin + coordinator + you)

The human admin has authority to give you instructions.
...
1. Read the task spec (usually at the shared path `shared/tasks/{task-id}/spec.md`)
2. Create `plan.md` in the task directory before starting work
3. Execute the task, keeping all intermediate artifacts in the task directory
4. Write your result to `result.md` in the task directory
...
Update checkboxes and Notes as you progress.
