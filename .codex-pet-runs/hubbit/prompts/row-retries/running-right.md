Create Codex pet row `running-right` for `hubbit`: exactly 8 full-body frames in one horizontal strip on flat pure magenta #FF00FF.

Use the attached canonical base for identity and the layout guide only for spacing. Same pet in every frame: A compact plush-tech companion inspired by a curious AI builder: small rounded body, bright attentive eyes, tiny satchel/notebook, subtle circuit-stitch details, calm helpful personality, warm teal, amber, ink, and off-white palette, no text or logos.. Preserve silhouette, face, palette, material, proportions, markings, and props.
