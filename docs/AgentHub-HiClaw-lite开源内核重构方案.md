...
最后更新：2026-06-07
状态：重构总纲 / 轻量版 HiClaw 内核方向已确立，真实 Matrix、resident Manager/Worker 与 Controller Plane 已进入现场验收阶段
...
运行时规范化的详细目标见 [Agent运行时规范化与HiClaw对齐计划.md](./Agent运行时规范化与HiClaw对齐计划.md)。后续所有 Manager / Worker runtime base 都应逐步具备同一套 Agent contract：`SOUL.md`、`AGENTS.md`、`skills/`、`registry/state`、标准 workspace、heartbeat、Room membership、shared task contract 和 Controller reconcile。OpenClaw/QwenPaw、Claude Code、OpenCode、Codex、Gemini 可以有不同 adapter 和进程形态，但不能再是各走各的临时调用。
...
3. **旧路径已经删除或降级，不允许恢复。**
   - `OrchestratorEngine`、`TaskExecutionService`、`LocalA2ATransport` 已删除；`sessions.metadata.kind` 只允许作为会话投影兼容读取，新接口、新表、新任务不能继续以旧 metadata 为事实源。
   - 如果兼容成本超过新内核推进成本，直接清库重建。

4. **每个阶段必须能独立验收。**
   - Phase 1 验收真实 Matrix room/timeline/participant 写入，不要求所有 participant token 完成。
   - Phase 2 验收 Manager workspace、SOUL/skills、OpenClaw/QwenPaw bridge，不再默认内部 LLM。
   - Phase 3 验收 WorkerRuntime 从 task room 接单并写回 timeline，不要求 Matrix 外部 homeserver。
   - Phase 4 验收 shared task contract 和 artifact ref 全链路；MinIO/S3 adapter 是主目标，本地 object store 仅为 fallback。
...
```text
体验真实透明 > 真实开源通信/存储/运行时 > 内核边界清晰 > 可本地跑通 > 企业级完整性 > 旧数据兼容
```

## 0.2 当前实施进度

截至 2026-06-07，本轮 HiClaw-lite Kernel 重构已经从本地可验证切片推进到真实开源组件 bridge，并开始进入真实 Matrix + OpenClaw resident runtime 的现场验收：

- **2026-06-07 现场状态补充**：本机 HiClaw-lite runtime 已经可以通过设置页和 `bun run dev` 进入可实测状态。OpenClaw Manager 以本地 resident process 运行时，会使用自己的 Matrix identity 通过真实 `/sync` 加入 room；Manager workspace 位于 `%LOCALAPPDATA%/AgentHub/manager/global`，`openclaw.json`、`SOUL/AGENTS/TOOLS`、`agenthub-controller` skill、Controller API 可达性和 room binding 由设置页 Manager Runtime 诊断展示。旧 room 仍可能出现 `unbound`，这是历史 Manager participant 与当前 resident Manager identity 不一致导致的漂移，目标收口方式是 room reconcile / 重建 binding / 新建 room，不是继续混用旧身份。
- **Worker 创建和模型绑定已经补第一刀**：现场发现 `runtimeBase=opencode` 但 `modelId=null` 的 Worker 会进入 `failed: No model configured for this worker`，旧 group mention bridge 还曾把失败 Worker 误回写为 `idle`。现在 `ControllerApi.createWorker()` 要求新 Worker 必须显式绑定模型，或存在 `AGENTHUB_WORKER_LLM_MODEL / LLM_MODEL` 作为 Worker 模型来源；`WorkerRuntimeService.runGroupMentionRoom()` 会返回真实 runtime status，失败不再被覆盖成 idle。后续前端“添加 Worker”、Manager 补员确认和群聊组队都必须把 Worker runtime base、模型绑定、skills 和 sandbox 一起传到 Controller API。
- **Worker runtime 口径已收紧**：OpenCode / Claude Code / Codex / Gemini 目前仍是 AgentHub-managed Worker bridge，它们通过 Room timeline 输入输出，由 AgentHub 服务托管 CLI 子进程；OpenClaw Worker 是 HiClaw 式 resident Worker 目标形态，要求独立 Matrix identity、room membership、带 token 的 `openclaw.json`、长期 listener/gateway，并在被 @ 后自主回复或执行。UI、设置页和诊断必须持续标注 bridge/resident 区别。
...
- **Phase 2 已从 CoordinatorRuntime 壳转向 ManagerRuntime 主路径**：`CoordinatorRuntime` 只保留为显式测试/迁移兼容层，群聊主入口经 `CoordinatorService -> ManagerRuntimeService -> ManagerRuntimeProvider`。Manager 配置目录化为 `SOUL.md`、`AGENTS.md`、`skills/`、`workers-registry.json`、`state.json`；Manager workspace 会生成 AgentHub 版 skill 契约，每个 skill 都遵循“读 Matrix timeline -> 自然语言理解 -> 调 Controller API 改真实资源 -> 回写 Matrix room”。没有可用 Manager runtime 时写入明确 blocked/diagnostic timeline event，不再静默回退到内部 LLM 或旧 Planner。
- **运行时规范化已正式纳入目标架构**：HiClaw 的 `SOUL.md / AGENTS.md / skills / workers-registry.json / state.json / heartbeat / reconcile` 不再只是参考资料，而是 AgentHub Manager/Worker runtime 的目标 contract。OpenClaw/QwenPaw 是 resident runtime 优先实现；Claude Code / OpenCode / Codex / Gemini bridge 也必须接入同一套 workspace、AGENTS 注入、heartbeat、RuntimeLease 和 ArtifactStore 语义。
...
- **Phase 4 已落地 filesystem-first ArtifactStore / SharedStorage bridge**：`artifacts` 表新增 `roomId`、`storageProvider`、`bucket`、`objectKey`、`storagePath`；`registerTaskArtifact()` 会按 S3-compatible object key 语义把文件、内联内容或 artifact descriptor materialize 到本地 object store，配置 `AGENTHUB_OBJECT_STORE_PROVIDER=s3` 时可切到 S3/MinIO。新增 `SharedStorage` 会发布 `shared/tasks/{taskId}/meta.json`、`spec.md`、`plan.md` 和终态 `result.md`；即使没有项目工作区，也能通过 object refs 完成 Room @mention + 共享文件交接。
...
- **2026-06-05 至 2026-06-07 Room / Matrix mention 主线补强**：新群聊、任务子对话、直聊 Agent 回复、Manager/Worker 过程消息不再新增 `messages` 投影行；API/WS 从 Room timeline 投影出 `room:{timelineEventId}` 显示消息，`messages` 只保留历史旧会话只读兼容。运行详情页 retry、ManagerLoop prepared-task 派发、Coordinator assign 和动态计划分发都统一为“写 task room Matrix mention-first timeline event -> `MatrixRoomEventDispatcher` -> Worker claim -> `WorkerRuntimeService.runTaskRoom()`”。`WorkerRuntimeService.rerunTaskRoom()` 只保留为低层兼容/测试 helper，不再作为生产路由或 ManagerLoop 主调度入口。终态 Manager final review 只写 group room timeline，`orchestrator_runs.summaryMessageId` 不再保存 `room:{eventId}`。
...
- 前端已经具备 `/api/rooms` 的最小读取、replay 投影、`room:timeline` realtime WS 增量投影，以及从 Room task event 自举最小任务看板/子对话入口的能力；ManagerPatrol 的监督事件也已进入 Room timeline。主视图正在收敛为 Room session snapshot + Resource snapshot；旧 messages、旧 AG-UI replay 和 run snapshot 只允许用于历史/调试页，不应再驱动当前聊天页状态。
- 真实 Matrix adapter 已接入第一版，并具备 MatrixIdentity、真实用户注册/登录、invite/join reconcile、每 participant token 发言、listener supervisor、mention dispatch、基础控制消息和 file refs -> ArtifactStore。仍缺的是 TokenVault/加密存储、typing/presence、更完整 Matrix media 到 MinIO/S3 的现场验证、OpenClaw/QwenPaw 原生 Matrix runtime 长期稳定性、完整 room membership 状态机和前端更 Matrix-sync-native 的投影。
...
- WorkerRuntime 已经被 Coordinator assign 主线、动态计划初始分发、运行详情页单任务 retry 和 ManagerLoop prepared-task 原生派发调用，并支持进度、消息、失败、澄清请求、partial artifact 的 task room timeline 写回；澄清请求已经从纯 timeline metadata 提升为 `task_clarifications` pending/answered 资源，且会同步到 TaskThread / RuntimeLease / WorkerInstance / RunEvent 的 `waiting_for_human` 资源状态。用户回复澄清后的最小 resume path 与失败后 rerun path 已经打通。但 OpenCode / Claude / Codex / Gemini 仍主要是 AgentHub-managed bridge；OpenClaw Worker resident path 已有 identity/config/listening 闭环，仍要继续现场验证“Worker 自己监听 Matrix room、被 @ 后自主接单、等待/恢复不依赖 service 代跑”的完整 HiClaw 式生命周期。
- RuntimeLease 已经在 Coordinator assign、WorkerRuntime、ManagerLoop prepared-task 派发、启动/关闭恢复中统一走 `RuntimeLeaseController`。ManagerPatrol 的 heartbeat stale 检测现在只发 warning 和 Room timeline 监督事件，不再直接 stale lease；旧 Engine 已删除，后续任何真正改变 lease 终态的 lifecycle 能力都不能绕过 `RuntimeLeaseController`。
...
- 专家配置、模型配置、Agent Runtimes / Agent Bases、Skills/MCP 能力配置。旧 UI 中的 `Coding Tools` 只是历史命名。
...
4. **共享存储层**：对应 [hiclaw-wiki.agent.final.md](./hiclaw-wiki.agent.final.md) 第 7 章。默认主路径采用本地 filesystem SharedStorage，但目录和 object key 语义按 MinIO/S3-compatible 设计。所有任务输入、结果、artifact、handoff ref 都进入统一 ArtifactStore / SharedStorage，不再散落在消息 metadata 或黑板里；后续可替换为 MinIO/S3 adapter。
...
- `OrchestratorEngine` 已删除；Manager 决策和执行生命周期只能继续进入 `CoordinatorRuntime` / `ManagerLoop` / `RunController` / `WorkerRuntimeService`。
...
### 4.4 共享存储：filesystem first，保持 S3-compatible 语义

决策：**本地 filesystem object store 是第一阶段默认主路径；MinIO/S3-compatible 是接口语义和后续可替换 adapter，不是默认重依赖。**
...
LocalFilesystemArtifactStore
  仅作为 fallback / mirror
  root = 用户数据目录 / AgentHub / storage / objects
  shared task object keys = shared/tasks/{taskId}/{meta.json|spec.md|plan.md|result.md|artifacts/...}
```
...
- MinIO/S3 adapter 保留为后续部署增强；开发期可以一键 compose 启动，但不要求作为默认依赖。
- filesystem 是默认轻量实现；架构真相是 ArtifactStore / SharedStorage contract，而不是某个具体存储后端。
...
使用 MinIO/S3 时，以上路径就是 object key，不需要重写上层业务。
...
- 旧 `TaskScheduler` 已删除；recovery / retry / replan 分支必须走资源控制面。
...
| `OrchestratorEngine` 总控 | ManagerRuntime + RunController + WorkerController | 已删除，禁止恢复 |
| `Planner` 主脑 | Manager planning skill | 不再主路径调用 |
| `TaskScheduler` 总调度 | Coordinator assign + RunController + ManagerLoop + WorkerRuntimeService | 已删除，禁止恢复 |
...
- WorkerBackend seam 区分 Local CLI、OpenClaw/QwenPaw resident Worker、本地 OpenClaw resident process、Docker resident Worker 和 Docker sandbox Worker。
...
```text
typing / presence
人工确认与 pending proposal / approval request 的强绑定
OpenClaw/QwenPaw 原生 Matrix runtime 进程化
Matrix identity TokenVault / secret redaction
