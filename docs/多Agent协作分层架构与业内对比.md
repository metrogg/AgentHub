...
本文用于统一 AgentHub 的长期架构判断：我们要做的不是一个固定角色模板系统，也不是只暴露后端图编排 API 的框架，而是以多 Coding Agent 协作为运行时内核，逐步长成 `Coze 风格的开源 AI 工作平台`。

当前工程事实总览见 `docs/当前状态与下一步路线.md`，产品北极星与对标拆解见 `docs/Coze新版本对标拆解与开源复刻路线.md`。本文侧重“为什么这样分层”和“和业内方案相比差在哪里”。
...
而从产品形态上，我们现在的这条主路径应该被理解成：

- 近期：IM 风格的多 Agent 工作流。
- 中期：Coze 风格的 `Space + Task + Asset + Expert + Eval` 工作平台。
...
- Coze：把“AI 团队、项目、文件、技能商店、Agent World、云端沙箱、部署”组织成一套统一工作平台叙事。
...
- 我们还没有形成 Coze 那样的一级信息架构，当前仍然更依赖“会话树”承载产品结构。
