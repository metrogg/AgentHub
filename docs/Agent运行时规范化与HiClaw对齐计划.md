...
4. **Manager skill migration**
   ...
   - 进展：`applyControllerManifest()` 现在会为每个 manifest 返回 `approval` 和 `audit` 摘要。`audit` 包含 resource operation id、`apply.manifest` 入口、danger、manifest kind/name 和按 Controller schema audit 字段抽取出的值；`approval` 暴露 schema level、是否 required、是否提供 approval token/说明。当前先做到结果可见和可写回 Room timeline，后续再接持久审计表与产品态确认卡。
   ...
   - 剩余：把 apply approval/audit 摘要持久化，补真正的 required approval 产品态确认，升级为完整 OpenAPI/JSON schema，以及 Team Leader 这类复合 Manager/Worker 资源。
