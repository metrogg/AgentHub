...
4. **Manager skill migration**
   ...
   - 进展：`POST /api/controller/apply` 已从 stub 变成真实 manifest apply；`agenthub apply -f <file>` 可以提交 JSON / 轻量 YAML，当前支持 `Worker`、`Room`、`Task`、`Team` 和 `Human`。Worker 进入 Member Reconcile，Room 进入 Matrix-backed Room 创建，Task 进入 task room / RuntimeLease / Matrix @mention 派发链路，Team 会组已有 Worker 并确保团队 room participant，Human 会进入一等 human identity 创建流程。第一轮严格校验已接入：Worker 必须有显式 `runtimeBase` 和 `modelId`，runtime enum 包含 `qwenpaw/copaw`，Room kind 必须属于 Controller schema enum，`sandboxPolicy.mode` 对象形态会被正确投影。
   ...
   - 剩余：补危险操作 approval、持久审计字段、完整 OpenAPI/JSON schema，以及 Manager 资源 manifest。
