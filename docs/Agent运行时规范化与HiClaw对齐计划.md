...
4. **Manager skill migration**
   ...
   - 进展：Controller apply 审计摘要已持久化到 `controller_audit_events`。每个 apply item 会写入 operation id、danger、approval level/provided、manifest kind/name、workspace/resource id、audit fields 和 result summary，并把 `auditEventId` 返回给 Manager skill。这样 Manager 的“自然语言 -> skill -> Controller API”不再只有房间消息，控制面也有可追踪审计记录。
   ...
   - 剩余：补 Controller audit 查询接口、真正的 required approval 产品态确认/确认卡，升级为完整 OpenAPI/JSON schema，以及 Team Leader 这类复合 Manager/Worker 资源。
