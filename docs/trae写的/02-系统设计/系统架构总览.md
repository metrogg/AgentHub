...
graph TB
    ...
    subgraph INFRA["基础设施层 Infrastructure Layer"]
        ...
        PreviewSB["Preview Sandbox<br/>Sandpack/WebContainer"]
...
| 预览面板 | Sandpack 浏览器沙箱渲染（CodeSandbox 开源技术）、多设备视口切换（桌面/平板/手机）、HMR 热更新、TypeScript 编译 | 详见 [开源参考](./开源项目参考与复用分析.md#6-网页预览沙箱) |
...
| Preview Sandbox | Sandpack 浏览器沙箱（iframe + sw 隔离）、postMessage 通信、多视口模拟、TypeScript 编译 | `@codesandbox/sandpack-react` |
...
| 维度 | 说明 |
|------|------|
| **输入** | Agent 修改的文件内容（`PreviewFile[]`：path、content、type） |
| **处理** | 通过 Sandpack Template 组装多文件项目 → Sandpack 浏览器沙箱编译渲染 |
| **输出** | 可视化的页面预览（支持 TypeScript 编译、npm 依赖、HMR 热更新） |
...
interface WsMessage {
  ...
  timestamp: string;  // ISO 8601 UTC 格式
}
```
...
- 指数退避因子：2x
- 最大重连延迟：30s
- 重连成功后自动发送 `AUTH` 消息重新认证
- 认证成功后通过 REST API 补拉断线期间的消息（`GET /api/v1/sessions/{session_id}/messages?after=last_message_id`）
...
- 响应格式：`{ code: number, message: string, data: T, request_id: string }`
- 认证方式：`Authorization: Bearer <jwt_token>`
- 分页标准：消息列表使用 Cursor 分页（`?cursor=xxx&limit=50`），管理列表使用 Offset 分页（`?page=1&page_size=20`）
...
| `GET` | `/api/v1/sessions` | 获取会话列表（Offset 分页） |
...
| `POST` | `/api/v1/sessions/{session_id}/diffs/{diff_id}/accept` | 接受 Diff |
| `POST` | `/api/v1/sessions/{session_id}/diffs/{diff_id}/reject` | 拒绝 Diff |
