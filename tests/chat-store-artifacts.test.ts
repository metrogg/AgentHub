...
describe('chat store artifact snapshot projection', () => {
  ...
  test('contract: AG-UI plan, task and artifact events project into the task board', () => {
    let state = {
      sessions: [
        session({
          id: 'group-1',
    ...
    state = __chatStoreTestHooks.applyAgUiEventToState(
      state,
      {
        type: 'CUSTOM',
            ...
            tasks: [
              {
                ...
                phaseId: 'implementation',
                title: 'Build page',
                description: 'Create the static landing page',
                agentId: 'agent-1',
                dependencies: [],
              },
            ],
    ...
    expect(state.taskBoard).toMatchObject({
      runId: 'run-1',
      sessionId: 'group-1',
      status: 'planning',
      title: 'Launch plan',
    ...
    expect(state.taskBoard?.phases[0]).toMatchObject({
      id: 'implementation',
      status: 'pending',
      taskIds: ['task-1'],
    })
    expect(state.taskBoard?.tasks[0]).toMatchObject({
      id: 'task-1',
      agentId: 'agent-1',
      agentName: 'Builder',
      status: 'pending',
    ...
    expect(state.agentTabs[0]).toMatchObject({
      taskId: 'task-1',
      agentName: 'Builder',
      status: 'pending',
      childSessionId: null,
    ...
    expect(state.taskBoard).toMatchObject({ status: 'running' })
    expect(state.taskBoard?.phases[0]).toMatchObject({ id: 'implementation', status: 'active' })
    expect(state.taskBoard?.tasks[0]).toMatchObject({
      id: 'task-1',
      status: 'running',
      progress: 40,
    ...
    const taskSession = state.sessions.find((item: Session) => item.id === 'child-1')
    ...
    expect(taskSession?.metadata).toMatchObject({
      kind: 'orchestrator-task',
      groupSessionId: 'group-1',
      orchestratorRunId: 'run-1',
      orchestratorTaskId: 'task-1',
    ...
    expect(state.taskBoard?.tasks[0]?.artifactCount).toBe(1)
    expect(state.taskBoard?.tasks[0]?.artifacts).toEqual([
      expect.objectContaining({
        artifactId: 'artifact-1',
        title: 'index.html',
        filePath: 'dist/index.html',
