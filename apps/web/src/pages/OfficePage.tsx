        ...
        <aside className="flex h-full w-72 shrink-0 flex-col border-r border-neutral-200 bg-white">
          ...
          <div className="min-h-0 flex-1 overflow-y-auto p-4">
            {activePanel === 'workspace' ? (
              <section>
                <div className="text-sm font-semibold text-neutral-900">当前工作区</div>
                <div className="mt-3 rounded-lg border border-neutral-200 bg-[#fafaf7] p-3">
                  <div className="truncate text-sm font-medium text-neutral-950">
                    {currentWorkspace?.name ?? '未绑定工作区'}
                  </div>
                  <div className="mt-1 truncate text-sm text-neutral-500" title={currentWorkspace?.projectPath ?? ''}>
                    {currentWorkspace?.projectPath ?? 'Star Office 将独立运行'}
                  </div>
                </div>

                <div className="mt-5 flex items-center justify-between">
                  <div className="text-sm font-semibold text-neutral-900">工作区列表</div>
                ...
                <div className="mt-3 space-y-2">
                  ...
                  {workspaces.map((workspace) => (
                    <WorkspaceButton
                      ...
                      subtitle={workspace.projectPath ?? '未设置项目文件夹'}
              ...
              <section>
                ...
                <div className="mt-5 rounded-lg border border-neutral-200 bg-[#fafaf7] p-3">
                  ...
                  <p className="mt-2 text-sm leading-6 text-neutral-600">
                    默认验证码是 <span className="font-semibold text-neutral-950">1234</span>。长期运行时建议在 Star Office
                    后台改为强密码。
                  </p>
                </div>

                <div className="mt-5 rounded-lg border border-neutral-200 bg-[#fafaf7] p-3">
                  <div className="text-sm font-semibold text-neutral-900">服务地址</div>
            ...
            {!loaded && (
              <div className="absolute inset-0 z-10 grid place-items-center bg-white/90 backdrop-blur-sm">
                <div className="w-[22rem] rounded-lg border border-neutral-200 bg-white p-6 text-center shadow-xl">
                  ...
                  <div className="mt-4 text-base font-semibold text-neutral-950">正在打开 Star Office</div>
                  <p className="mt-2 text-sm leading-6 text-neutral-500">
                    后端应运行在 {STAR_OFFICE_URL}。如果长时间停留，请确认 Star Office 后端已启动。
                  </p>
                </div>
              </div>
            )}
