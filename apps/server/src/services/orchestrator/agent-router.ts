...
interface SelectAgentInput {
  task: Pick<ExecutionTask, 'id' | 'title' | 'description' | 'agentId' | 'taskType' | 'dependencies' | 'maxRetries'>
...
export function selectAgentForTask(input: SelectAgentInput): AgentSelection {
  ...
  const scored = candidates
    .filter((agent) => isEligible(agent, task.taskType))
    .map((agent) => scoreAgent(agent, task, relations))
    .sort((a, b) => b.score - a.score)

  const selected = scored[0] ?? scoreAgent(candidates[0]!, task, relations)
  const reviewer = findRelatedAgent(selected.agent, 'reviewed_by', agents, relations)
...
function scoreAgent(agent: ExecutionAgent, task: SelectAgentInput['task'], relations: AgentRelation[]) {
  ...
  const text = [agent.name, agent.role, agent.description, agent.runtimeType, agent.codeAgentType, ...tags]
    .filter(Boolean)
    .join(' ')
    .toLowerCase()
  let score = 0
  ...
  if (relations.some((relation) => relation.sourceAgentId === agent.id || relation.targetAgentId === agent.id)) {
    score += 10
...
function findRelatedAgent(
  ...
  const relation = relations.find((item) => item.sourceAgentId === source.id && item.relationType === relationType)
