...
export class NativeToolRuntime implements AgentRuntime {
  readonly runtimeType = 'mcp'
  readonly displayName = '原生只读工具'

  async *execute(ctx: ExecutionContext): AsyncGenerator<AgentOutputChunk> {
    const { profile, prompt, history, signal, workspacePath } = ctx
    const cwd = workspacePath ?? env.AGENTHUB_WORKSPACE_ROOT

    if (!cwd) {
      yield { kind: 'text', text: '原生只读 Agent 无法启动：项目目录无效。' }
      return
    }

    const config = await resolveLlmRuntimeConfig(profile.modelId ?? undefined)
    if (!config.apiKey) {
      yield { kind: 'text', text: 'API Key 未配置。请在环境变量中设置 LLM_API_KEY 或 OPENAI_API_KEY 等供应商专用 Key，然后重启服务。' }
      return
    }

    const permissions = normalizePermissions(profile.toolPermissions)
    const defaultReadOnly =
      profile.runtimeType === 'mcp' &&
      (permissions.length === 0 || (permissions.length === 1 && permissions.includes('chat')))

    const allowedTools = readOnlyToolRegistry.allowedTools(
      defaultReadOnly ? ['workspace:read', 'skills:read'] : permissions,
      { readOnlyOnly: true },
    )

    const skillContext = await globalSkillRegistry.buildSkillContext(
      [profile.systemPrompt, profile.description, prompt, history.slice(-6).map((item) => item.content).join('\n')]
        .filter(Boolean)
        .join('\n\n'),
      { capabilityTags: profile.capabilityTags, limit: 2 },
    )

    const system = buildNativeSystemPrompt(profile, cwd, allowedTools, skillContext)
    ...
    try {
      const output = isAnthropicProvider(config)
        ? await runAnthropicNativeLoop(config, system, messages, allowedTools, cwd, signal)
        : await runOpenAINativeLoop(config, system, messages, allowedTools, cwd, signal)

      yield { kind: 'text', text: output.trim() || '（原生 Agent 已完成，但没有返回文本内容。）' }
    } catch (error: any) {
...
async function runAnthropicNativeLoop(
  ...
  for (let round = 0; round < env.AGENTHUB_NATIVE_MAX_TOOL_ROUNDS; round += 1) {
    ...
    for (const toolUse of toolUses) {
      ...
      toolResults.push({ type: 'tool_result', id: toolUse.id, content: result })
    }
...
function buildNativeSystemPrompt(profile: AgentProfile, cwd: string, tools: ToolDefinition[], skillContext: string) {
  return [
