  ...
  operations: [
    ...
    },
    {
      ...
      body: {
        ...
        approvalMode: field('string', false, 'Use request to write a Room-native approval.requested event without applying yet.', ['apply', 'request']),
        requestApprovalRoomId: field('string', false, 'Room id that receives approval.requested when approvalMode=request.'),
        approvalRequest: field('object', false, 'Optional { roomId, reason, requestedBy } metadata for Room-native approval requests.'),
        approval: field('object', false, 'Optional { approved, approvedBy, reason } used when applying an already approved request.'),
      },
    },
    {
      id: 'apply.approval_confirm',
      skill: 'human-management',
      method: 'POST',
      path: '/api/controller/approvals/{eventId}/confirm',
      summary: 'Confirm a Room-native Controller apply approval request and then apply its stored manifests with audit metadata.',
      danger: 'write',
      approval: 'not_required',
      audit: ['eventId', 'approvedBy', 'reason'],
      body: {
        approvedBy: field('string', false, 'Human or Manager identity that confirmed the request.'),
        reason: field('string', false, 'Approval reason visible in audit metadata.'),
      },
    },
    {
      id: 'apply.approval_deny',
      skill: 'human-management',
      method: 'POST',
      path: '/api/controller/approvals/{eventId}/deny',
      summary: 'Deny a Room-native Controller apply approval request without applying its stored manifests.',
      danger: 'write',
      approval: 'not_required',
      audit: ['eventId', 'deniedBy', 'reason'],
      body: {
        deniedBy: field('string', false, 'Human or Manager identity that denied the request.'),
        reason: field('string', false, 'Denial reason visible in Room timeline.'),
      },
    },
    {
      id: 'audit.list',
