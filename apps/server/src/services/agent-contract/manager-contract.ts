...
function buildManagerTools(): string {
  return [
    ...
    '- Controller health check is `${AGENTHUB_CONTROLLER_URL}/health`; do not call `/api/health`.',
    '- Use AGENTHUB_MANAGER_TOKEN as the bearer token when available.',
    '- On Windows PowerShell, do not send non-ASCII JSON with `Invoke-RestMethod -Body $json` directly; send UTF-8 bytes instead: `$bytes = [Text.Encoding]::UTF8.GetBytes($json)` and use `-Body $bytes -ContentType "application/json; charset=utf-8"`.',
    '- If a Controller response shows names or roles as `????`, stop and report an encoding failure instead of claiming success.',
...
function buildManagerContext(input: EnsureManagerAgentContractInput): string {
  ...
  return [
    ...
    '- Controller health check path: /health (not /api/health).',
...
function fallbackSkillDoc(name: string): string {
  return [
    ...
    '- Controller health check is `${AGENTHUB_CONTROLLER_URL}/health`; do not call `/api/health`.',
    '- On Windows PowerShell, send non-ASCII JSON bodies as UTF-8 bytes (`[Text.Encoding]::UTF8.GetBytes($json)`) with `Content-Type: application/json; charset=utf-8`.',
    '- If a created resource comes back with `????` in a name, role, description, or prompt, treat it as a failed action and ask for recovery.',
    '',
    '## Decision Pattern',
