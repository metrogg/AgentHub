...
AgentHub 是一个面向多 Agent 协作的本地工作台。它把 Claude Code、Codex、OpenCode 等 Code Agent 抽象成统一的聊天对象，让用户可以在一个类似 IM 的界面里创建单聊、群聊、工作区和产物卡片，并逐步把代码、文档、网页预览、技能和部署流程串起来。
...
- **Code Agent 接入**：统一管理 Codex、Claude Code、OpenCode 的运行参数和状态，支持 CLI 执行、过程日志、命令记录、文件变更和产物卡片。
...
- Codex：读取和编辑本机 `config.toml`、`auth.json`，可作为 Code Agent 执行任务。
- Claude Code：读取配置并作为 Code Agent 默认运行参数。
